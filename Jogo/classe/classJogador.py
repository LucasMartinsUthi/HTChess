...
class classJogador:
	...
	def socket(self, turno = False):
		...
		arr = json.dumps({'id': self.id, 'lenMao' : len(self.mao), 'mesa' : mesa, 'lenDeck': len(self.deck), 'vida': self.vida, 'mana': self.mana, 'manaTotal': self.manaTotal, 'drag': self.drag, 'cartaSelecionada': self.cartaSelecionada}, ensure_ascii=False).encode('utf8')
		con.send(arr)
		...
		if self.inimigo != None and list(self.inimigo.keys()) != ['turno']:
			...
			for i in range(mao):
				if i != self.inimigo['cartaSelecionada']:
					...
					# charImage = pygame.image.load("img/backcard.png")
					# charImage = pygame.transform.scale(charImage, cartaInimiga.size)
					# charImage = charImage.convert()
					# self.canvas.blit(charImage, cartaInimiga)
				pos[0] += 120
